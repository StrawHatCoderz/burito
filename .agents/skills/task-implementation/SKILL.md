...
description: >
  Single-task implementation skill. Given a specific task from docs/<story_id>/tasks.md,
...
- The task definition from `docs/<story_id>/tasks.md` — description, acceptance
...
> "This feedback suggests the problem may be outside this task's scope:
...
Story: <story_id> — Task <N>/<total> - <task title>"
```
...
| `Story` | Always include — `Story: <story_id> — Task <N>/<total> - <task title>` |
