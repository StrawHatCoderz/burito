...
name: task-creation
description: >
  Task breakdown skill for an agreed story plan. Reads docs/<story_id>/plan.md and
  decomposes it into discrete, independently-implementable technical tasks with acceptance
  criteria. Saves the result to docs/<story_id>/tasks.md. Used by the
...
- `docs/<story_id>/plan.md` exists and the user has agreed to it
...
> 3. Defer as a follow-up story card"
...
```bash
# Write task list to docs/<story_id>/tasks.md
```
...
> "Tasks saved to `docs/<story_id>/tasks.md`."
