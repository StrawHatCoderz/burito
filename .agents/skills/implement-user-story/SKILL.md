...
description: >
  Use this skill whenever the user provides a story card ID (in the format BR-003 or just
  a number) and wants to implement it. Orchestrates the full lifecycle: read requirements,
  analyse code, collaborative planning, task breakdown, and incremental implementation with
  tests and commits. Trigger any time the user says things like "implement BR-003",
  "work on story 15", "let's build BR-007", "pick up this card", or hands you a story ID
  with intent to write code.
---

# User Story Implementation — Orchestrator

This skill is the **entry point and conductor** for implementing a story card end-to-end.
...
│
├── Phase 0: Read & Analyse      ← load skills/issue-context/SKILL.md
│
...
Before loading the sub-skill, resolve the story ID from the user's input and set up the working directory:

```bash
bash .claude/skills/implement-user-story/scripts/setup-story-dir.sh <story_id>
```

If the directory already exists and contains files (`plan.md`, `tasks.md`, or others),
**stop and ask the user** before proceeding:

> "`docs/<story_id>/` already exists with the following files: \<list\>.
...
If the user chooses **Start fresh**: `rm -f "docs/<story_id>"/*.md` then continue normally.
...
> It will read the requirements file, survey the codebase,
> and produce a structured Context Summary.

The issue-context skill will:
- Read the requirements file from `requirements/<story_id>.md`
- Survey repo structure, relevant files, test conventions, and tech stack
- Produce a Context Summary, confirm it with the user, and save it to `docs/<story_id>/context.md`

**→ Phase 0 gate met when:** `docs/<story_id>/context.md` exists and the user has confirmed its contents.
**→ Then proceed to Phase 1.** The planning skill will read context from `docs/<story_id>/context.md` directly.
...
> It will read context from `docs/<story_id>/context.md`.

The planning skill will:
- Run the clarification/assumption/plan/agreement loop
- Save the agreed plan to `docs/<story_id>/plan.md`
...
- Save the task list to `docs/<story_id>/tasks.md`
...
Read `docs/<story_id>/tasks.md` to get the full task list and their dependency order.
...
1. Run the full test suite one final time to confirm nothing broke across all tasks:
   ...
   ```
2. Append a completion note to `docs/<story_id>/plan.md`:
   ```markdown
   ## Status: Completed
   Implemented in <N> tasks. All tests passing. Final commit: <hash>.
   ```
 3. Give the user a brief summary of what was built.
...
If `requirements/<story_id>.md` does not exist, stop immediately and ask the user to
confirm the story ID and check that the file is present before proceeding.
