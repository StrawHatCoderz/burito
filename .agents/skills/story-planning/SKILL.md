...
name: story-planning
description: >
  Collaborative planning skill for a story card. Takes issue content and codebase context
  as input. Runs an iterative clarification loop until the user explicitly agrees to a plan,
  then saves it to docs/<story_id>/plan.md. Used by the user-story-implementation skill —
...
the user says "Agreed" (or equivalent). Save the result at `docs/<story_id>/plan.md`.
...
```bash
# docs/<story_id>/ already exists (created in Phase 0a)
# Write the agreed plan to docs/<story_id>/plan.md
```

Confirm to the user:
> "Plan saved to `docs/<story_id>/plan.md`."
...
Suggest splitting into sub-stories in the requirements folder to draft them, then plan each separately.
